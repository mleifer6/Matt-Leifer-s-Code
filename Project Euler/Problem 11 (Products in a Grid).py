...
for r in range(20):
    for c in range(16):
        prod=M[r][c]*M[r][c+1]*M[r][c+2]*M[r][c+3] #horizontal products
        if prod>max_product:
            max_product=prod
for r in range(16):
    for c in range(20):
        prod=M[r][c]*M[r+1][c]*M[r+2][c]*M[r+3][c] #vertical products
        if prod>max_product:
            max_product=prod

for r in range(16):
    for c in range(16):
        prod=M[r][c]*M[r+1][c+1]*M[r+2][c+2]*M[r+3][c+3] #diangonals with slope -1
        if prod>max_product:
                max_product=prod
...
for r in range(3,20):
    for c in range(16):
        ...
        prod=M[r][c]*M[r-1][c+1]*M[r-2][c+2]*M[r-3][c+3] #diagonals with slope 1 
        if prod>max_product:
                max_product=prod
print max_product
